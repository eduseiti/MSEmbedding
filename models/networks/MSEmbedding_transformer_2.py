...
class MSEmbeddingTransformer2Net(nn.Module):
    ...
    def forward(self, batch):
        ...
        classification_begin = torch.empty(transform.shape[0], 1, transform.shape[2])
        classification_end = torch.empty(transform.shape[0], 1, transform.shape[2])
        classification_begin.fill_(self.classificationToken)
        classification_end.fill_(self.classificationToken)

        x = torch.cat([classification_begin.cuda(), transform, classification_end.cuda()], dim=1)
        ...
        for i in range(x.shape[0]):
            keyPaddingMask[i, originalPeaksLen[i]] = self.classificationToken
            keyPaddingMask[i, (originalPeaksLen[i] + 1):] = True
